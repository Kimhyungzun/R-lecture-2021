...
```{r}
      ggplot(mpg, aes(x=cty,y=hwy,col=class)) +
      geom_point()
```
...
ggplot(mid_asian, aes(x=poptotal,y=popasian)) +
...
```{r}
suv_cty <- filter(mpg, class=='suv') %>%
      group_by(manufacturer) %>%
      summarise(cty_avg=mean(cty)) %>%
   head(5)
ggplot(suv_cty, aes(reorder(manufacturer,cty_avg),cty_avg,fill=manufacturer)) +
...
mpg %>%
      group_by(class) %>%
      summarise(class_num=length(class)) %>%
      ggplot(aes(reorder(class,class_num),class_num)) +
...
diamonds %>%
      group_by(cut) %>%
      summarise(cut_num=length(cut)) %>%
      ggplot(aes(cut,cut_num)) +
      geom_bar(stat = 'identity',aes(fill=cut)) +
      labs(title = "cut's 개수", x= "cut",y="개수") 
```
...
cut_ideal <- filter(diamonds, cut=='Ideal')
cut_Verygood <- filter(diamonds, cut=='Very Good')
cut_good <- filter(diamonds, cut=='Good')
cut_fair <- filter(diamonds, cut=='Fair')
ggplot(cut_premium, aes(color,price,fill=color)) +
      geom_boxplot() +
      labs(title="premium의 색에따른 가격변화") +
      ylim(0,6000)
ggplot(cut_ideal, aes(color,price,fill=color)) +
      geom_boxplot() +
      labs(title="ideal의 색에따른 가격변화") +
      ylim(0,6000)
ggplot(cut_Verygood, aes(color,price,fill=color)) +
      geom_boxplot() +
      labs(title="Verygood의 색에따른 가격변화") +
      ylim(0,6000)
ggplot(cut_good, aes(color,price,fill=color)) +
      geom_boxplot() +
      labs(title="good의 색에따른 가격변화") +
      ylim(0,6000)
ggplot(cut_fair, aes(color,price,fill=color)) +
      geom_boxplot() +
      labs(title="fair의 색에따른 가격변화") +
       ylim(0,6000)
